export default function SocialProof() {
    ...
    return (
        <div className="bg-slate-100 dark:bg-[#0F1623] border-y border-slate-200 dark:border-white/5 py-10 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <p className="text-center text-slate-500 dark:text-slate-400 font-display uppercase tracking-widest text-xs mb-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 items-center opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
                    {companies.map((company) => (
                        <div key={company.name} className="flex justify-center">
                            <span className={`text-2xl text-slate-700 dark:text-white ${company.className}`}>
                                {company.name}
                            </span>
                        </div>
                    ))}
