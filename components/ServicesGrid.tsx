        ...
        <section className="py-24 bg-slate-50 relative">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                    <div>
                        <h2 className="font-display text-4xl md:text-5xl font-bold text-slate-900 uppercase mb-4">
                            Áreas de <span className="text-primary">Impacto</span>
                        </h2>
                        <div className="h-1 w-24 bg-primary"></div>
                    </div>
                    <p className="max-w-md text-slate-600 font-body text-lg text-right md:text-left">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {services.map((service) => (
                        <div
                            key={service.title}
                            className="group bg-white border border-slate-200 p-8 hover:border-primary transition-all duration-300 shadow-md hover:shadow-2xl hover:shadow-primary/10 relative overflow-hidden"
                        >
                            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                <service.icon className="w-24 h-24 text-primary" strokeWidth={1} />
                            </div>
                            <div className="relative z-10">
                                <h3 className="font-display font-bold text-2xl text-slate-900 uppercase mb-4 group-hover:text-primary transition-colors">
                                    {service.title}
                                </h3>
                                ...
                                <Link
                                    href="#"
                                    className="inline-flex items-center text-sm font-bold uppercase tracking-wider text-slate-900 group-hover:text-primary transition-colors"
                                >
